...
const CertificateVerifier = () => {
  useEffect(() => {
    // Unconditionally redirect to the dedicated verification app
    const redirectUrl = "https://verify.teranis.in";
  ...
  return (
    <div className="flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-gray-950 via-black to-gray-900 px-4 text-white">
      <div className="relative flex w-full max-w-md flex-col items-center rounded-3xl border border-white/10 bg-white/5 px-8 py-10 text-center shadow-2xl backdrop-blur-xl">
        ...
        <p className="mt-6 text-xs uppercase tracking-[0.35em] text-cyan-300/70">Teranis Verification</p>
        <h1 className="mt-3 text-2xl font-semibold text-white">Redirecting</h1>
        <p className="mt-3 max-w-sm text-sm leading-6 text-gray-300">Opening certificate verification page.</p>
      </div>
    </div>
  );
};
