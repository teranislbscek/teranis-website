const eventsData = {
  "Technical Events": [
    {
      "title": "Debugging in Python",
      "date": "March 5, 2026",
      "location": "Auditorium",
      "contact": [
        {"name": "Umar", "phno": "9037868807"},
        {"name": "Pratheek Rao", "phno": "8078724204"}
      ],
      "formLink": "https://forms.gle/HRpAVx7ivgz4tAVV9",
      "description": {
        "overview": "Python Potholes is a thrilling debugging challenge where participants test their problem-solving skills by identifying and fixing errors in Python code.",
        "guidelines": [
          "Round 1 (Qualifiers - 30 mins) – Participants will debug basic and logical errors in Python code.",
          "Round 2 (Finals - 30 mins) – Advanced debugging challenges in Python."
        ],
        "evaluationCriteria": ["✅ Accuracy", "✅ Speed"]
      },
      "image": "/debugging.png"
    },
    {
      "title": "Code with ChatGPT",
      "date": "April 12, 2026",
      "location": "Computer Lab",
      "contact": [
        {"name": "Jacob", "phno": "9605980433"},
        {"name": "Archana", "phno": "9061076939"}
      ],
      "formLink": "https://forms.gle/DujUzmr8dj6sTBvx5",
      "description": {
        "overview": "Code with ChatGPT is a coding competition that challenges participants to solve programming problems using GPT Models.",
        "guidelines": [
          "The event is divided into three levels:",
          "1️⃣ Basic Level",
          "2️⃣ Intermediate Level",
          "3️⃣ Advanced Level",
          "(Each level contains a set of selected coding problems, which will be provided at the start of the competition.)"
        ],
        "evaluationCriteria": [
          "✅ Effective Use of ChatGPT – Your prompts matter! The better your prompt, the better your chances of winning! 😊",
          "✅ Code Accuracy",
          "✅ Code Readability"
        ]
      },
      "image": "/chatgpt.png"
    },
    {
      "title": "Fast Coding in C",
      "date": "May 10, 2026",
      "location": "Cyber Lab",
      "contact": [
        {"name": "Sidharth", "phno": "9074652995"},
        {"name": "Dhanya", "phno": "8590899868"}
      ],
      "formLink": "https://forms.gle/Dae5U4a35YJnmcoV6",
      "description": {
        "overview": "Fast Coding in C is a fast-paced coding competition where participants race against time to solve programming challenges using the C language.",
        "guidelines": ["Total Time limit: 1 hour", "3 Rounds with elimination"],
        "evaluationCriteria": [
          "✅ Accuracy of the Code",
          "✅ Time to Complete and Execute",
          "✅ Code Readability",
          "✅ Test Case Handling",
          "✅ Formatted Output",
          "✅ Creativity"
        ]
      },
      "image": "/fastcoding.png"
    },
    {
      "title": "Shuffle Code",
      "date": "July 15, 2026",
      "location": "Innovation Hub",
      "contact": [
        {"name": "Shivanand", "phno": "9778252544"},
        {"name": "Aleesha", "phno": "9037323965"}
      ],
      "formLink": "https://forms.gle/DGwx4WTzVBZe5qAc6",
      "description": {
        "overview": "The Shuffle Code Challenge is a unique coding competition where participants receive code in a randomly shuffled order and must rewrite it in the correct sequence.",
        "guidelines": ["Total Time limit: 1 hour", "3 Rounds with elimination"],
        "evaluationCriteria": [
          "✅ Code Accuracy",
          "✅ Speed of Completion",
          "✅ Code Readability",
          "✅ Formatted Output"
        ]
      },
      "image": "/shufflecode.png"
    },
    {
      "title": "Swap Code",
      "date": "August 20, 2026",
      "location": "Main Hall",
      "contact": [
        {"name": "Umar", "phno": "9037868807"},
        {"name": "Abhinav", "phno": "9778362356"}
      ],
      "formLink": "https://forms.gle/qdkmAih7kDwvpXLB9",
      "description": {
        "overview": "Swap Code is an exciting frontend web development coding challenge where participants collaborate in an unconventional way—by switching roles at regular time intervals.",
        "guidelines": [
          "Total Time limit: 1 hour",
          "Team Event",
          "Team of 2 members",
          "The code should contain HTML, CSS, and JS only.",
          "Switch roles every 5 minutes"
        ],
        "evaluationCriteria": [
          "✅ Code Accuracy",
          "✅ Speed of Completion",
          "✅ Code Readability",
          "✅ Formatted Output"
        ]
      },
      "image": "/swapcodeWeb.png"
    },
    {
      "title": "Schrödinger's Program",
      "date": "June 1, 2026",
      "location": "Tech Park",
      "contact": [
        {"name": "Yousuf", "phno": "9594769391"},
        {"name": "Rabeeba", "phno": "7594911684"}
      ],
      "formLink": "https://forms.gle/hiAkuvtWhhg2cm4y7",
      "description": {
        "overview": "Schrödinger's Program is a unique coding challenge designed to test participants' ability to create unpredictable yet logically explainable Python programs."
      },
      "image": "/shrodinger.png"
    }
  ],

  //   "Inter-Department Events": [
  //     {
  //       title: "Logo Quiz (Offline)",
  //       date: "March 15, 2026",
  //       location: "Main Hall",
  //       contact: "Michael - 9876543212",
  //       formLink: "#",
  //       description:
  //         "Test your knowledge by identifying logos from various industries.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Poster Presentation (Offline)",
  //       date: "March 22, 2026",
  //       location: "Seminar Hall",
  //       contact: "Emily - 9876543213",
  //       formLink: "#",
  //       description:
  //         "Showcase your creativity and ideas through a well-designed poster presentation.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Problem Solving (Offline)",
  //       date: "April 5, 2026",
  //       location: "Innovation Hub",
  //       contact: "Nathan - 9876543214",
  //       formLink: "#",
  //       description:
  //         "Engage in analytical and logical problem-solving challenges.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Photography (Online)",
  //       date: "May 18, 2026",
  //       location: "Online Submission",
  //       contact: "Olivia - 9876543215",
  //       formLink: "#",
  //       description: "Submit your best photographs based on the given theme.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Aptitude Test (Offline)",
  //       date: "June 10, 2026",
  //       location: "Exam Hall",
  //       contact: "Sophia - 9876543220",
  //       formLink: "#",
  //       description: "Assess your aptitude skills with challenging questions.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Spell-B (Offline)",
  //       date: "June 25, 2026",
  //       location: "Library Hall",
  //       contact: "Daniel - 9876543221",
  //       formLink: "#",
  //       description:
  //         "Compete in a spelling contest to test your vocabulary skills.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Geoguessr LBSCEK",
  //       date: "July 5, 2026",
  //       location: "Tech Hall",
  //       contact: "Ethan - 9876543222",
  //       formLink: "#",
  //       description:
  //         "Use your geographical knowledge to guess locations accurately.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Tech Memes",
  //       date: "July 20, 2026",
  //       location: "Online Submission",
  //       contact: "Lily - 9876543223",
  //       formLink: "#",
  //       description: "Create and submit the funniest tech-related memes.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Non-Stop Talking Challenge",
  //       date: "August 10, 2026",
  //       location: "Auditorium",
  //       contact: "Ryan - 9876543224",
  //       formLink: "#",
  //       description:
  //         "Talk for 2 minutes without hesitating or stopping on a given topic.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Write Disadvantages of Social Media (Online)",
  //       date: "August 25, 2026",
  //       location: "Online Submission",
  //       contact: "Sophia - 9876543225",
  //       formLink: "#",
  //       description:
  //         "Submit a well-structured write-up on the disadvantages of social media.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //   ],
  //   "Year-wise Events": [
  //     {
  //       title: "Debugging in C",
  //       date: "March 5, 2026",
  //       location: "Computer Lab",
  //       contact: "Daniel - 9876543220",
  //       formLink: "#",
  //       description:
  //         "Enhance your debugging skills in C with real-world problem-solving challenges.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Guess the Output in Python",
  //       date: "April 12, 2026",
  //       location: "Tech Hub",
  //       contact: "Sophia - 9876543221",
  //       formLink: "#",
  //       description: "Test your Python knowledge by predicting program outputs.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Data Structures Quiz in Mentee Meter",
  //       date: "May 10, 2026",
  //       location: "Online",
  //       contact: "Liam - 9876543222",
  //       formLink: "#",
  //       description:
  //         "Participate in an interactive data structures quiz on Mentee Meter.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Kon Banega Crorepati",
  //       date: "June 1, 2026",
  //       location: "Auditorium",
  //       contact: "Emma - 9876543223",
  //       formLink: "#",
  //       description:
  //         "A quiz competition inspired by KBC, testing knowledge across various domains.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //     {
  //       title: "Suggestions for Changes Required in B.Tech Syllabus (Online)",
  //       date: "June 20, 2026",
  //       location: "Online Submission",
  //       contact: "Olivia - 9876543224",
  //       formLink: "#",
  //       description:
  //         "Submit your valuable suggestions for improving the B.Tech syllabus.",
  //       image:
  //         "https://thinkexam.com/blog/wp-content/uploads/2022/04/Coding-Hackathon_1-1.jpg",
  //     },
  //   ],
};

export default eventsData;
